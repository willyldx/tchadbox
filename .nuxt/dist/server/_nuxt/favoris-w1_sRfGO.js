import { c as useSeoMeta, h as useFavoritesStore, f as useCartStore, i as useAuthStore, d as __nuxt_component_0 } from "../server.mjs";
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (unref(favoritesStore).isEmpty) {
        ...
        _push(`</div><h3 class="text-lg font-semibold text-[var(--color-text)] mb-2">Aucun favori</h3><p class="text-[var(--color-text-muted)] mb-6 max-w-md mx-auto"> Parcourez notre catalogue et cliquez sur le cœur pour sauvegarder vos produits préférés. </p>`);
        ...
        ssrRenderList(unref(favoritesStore).items, (item) => {
          ...
          _push(ssrRenderComponent(unref(ShoppingCart), { class: "w-5 h-5" }, null, _parent));
          _push(`</button></div><p class="text-xs text-[var(--color-text-muted)] mt-1">Ajouté ${ssrInterpolate(formatDate(item.addedAt))}</p></div></div>`);
        });
        _push(`<!--]--></div>`);
      }
      if (unref(authStore).isAuthenticated && !unref(favoritesStore).isEmpty) {
        ...
        _push(`<div><p class="text-sm text-green-800 font-medium">Favoris synchronisés</p><p class="text-sm text-green-600">Vos favoris sont sauvegardés sur votre compte et accessibles depuis tous vos appareils.</p></div></div>`);
      } else {
        _push(`<!---->`);
      }
      if (!unref(authStore).isAuthenticated && !unref(favoritesStore).isEmpty) {
...
//# sourceMappingURL=favoris-w1_sRfGO.js.map
