import { i as useAuthStore, a as useHead, l as __nuxt_component_0, d as __nuxt_component_0$1, _ as __nuxt_component_3, b as __nuxt_component_3$1 } from "../server.mjs";
  ...
  setup(__props) {
    ...
    const fetchDeliveries = async () => {
      ...
      try {
        ...
        deliveries.value = (data?.data || []).map((order) => normalizeOrder(order));
      } catch (error) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (unref(loading)) {
          ...
          _push(ssrRenderComponent(_component_NuxtLink, {
            ...
            default: withCtx((_, _push2, _parent2, _scopeId) => {
              if (_push2) {
                ...
                _push2(`<p${_scopeId}>${ssrInterpolate(formatAddress(delivery.shippingAddress))}</p></div><div class="mt-3 pt-3 border-t border-gray-100"${_scopeId}><p class="text-xs text-gray-500"${_scopeId}>${ssrInterpolate(delivery.items?.length || 0)} article(s)</p></div>`);
              } else {
                return [
                  ...
                  createVNode("div", { class: "mt-3 pt-3 border-t border-gray-100" }, [
                    createVNode("p", { class: "text-xs text-gray-500" }, toDisplayString(delivery.items?.length || 0) + " article(s)", 1)
                  ])
                ];
              }
            }),
        ...
        _push(`</div><p class="text-gray-500 font-medium">Aucune livraison</p><p class="text-sm text-gray-400 mt-1">${ssrInterpolate(unref(activeTab) === "pending" ? "Pas de nouvelles livraisons assignées" : "Aucune livraison dans cette catégorie")}</p></div>`);
      }
...
//# sourceMappingURL=index-uwE9n4AW.js.map
