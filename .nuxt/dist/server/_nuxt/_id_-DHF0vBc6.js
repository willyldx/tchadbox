import { k as useRoute, i as useAuthStore, c as useSeoMeta, d as __nuxt_component_0 } from "../server.mjs";
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(ssrRenderComponent(unref(ChevronRight), { class: "w-4 h-4 text-[var(--color-text-muted)]" }, null, _parent));
      _push(`<span class="text-[var(--color-text)] font-medium">${ssrInterpolate(unref(order)?.displayId || "Chargement...")}</span></nav>`);
      if (unref(isLoading)) {
        ...
        _push(`<div class="space-y-6"><div class="bg-white rounded-2xl border border-[var(--color-border)] p-6"><div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4"><div><div class="flex items-center gap-3 mb-2"><h1 class="text-2xl font-bold text-[var(--color-text)]">${ssrInterpolate(unref(order).displayId)}</h1><span class="${ssrRenderClass([getStatusClass(unref(order).fulfillmentStatus), "inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-sm font-medium"])}"><span class="${ssrRenderClass([getStatusDotClass(unref(order).fulfillmentStatus), "w-2 h-2 rounded-full"])}"></span> ${ssrInterpolate(getStatusLabel(unref(order).fulfillmentStatus))}</span></div><p class="text-[var(--color-text-muted)]"> Commandé le ${ssrInterpolate(formatDate(unref(order).createdAt))}</p></div><div class="flex gap-3"><button class="flex items-center gap-2 px-4 py-2 border border-[var(--color-border)] rounded-xl text-[var(--color-text-secondary)] hover:bg-gray-50 transition-colors">`);
        ...
        ssrRenderList(unref(order).items, (item) => {
          ...
          _push(`</div><div class="flex-1 min-w-0"><p class="font-medium text-[var(--color-text)]">${ssrInterpolate(item.title)}</p><p class="text-sm text-[var(--color-text-muted)]">Quantité: ${ssrInterpolate(item.quantity)}</p></div><p class="font-semibold text-[var(--color-text)]">${ssrInterpolate(formatPrice(item.total))}</p></div>`);
        });
        ...
        _push(`<span class="${ssrRenderClass([unref(order).paymentStatus === "captured" ? "text-green-700" : "text-amber-700", "text-sm"])}">${ssrInterpolate(unref(order).paymentStatus === "captured" ? "Paiement confirmé" : "Paiement en attente")}</span></div></div></div></div></div><div class="flex flex-col sm:flex-row gap-4">`);
        _push(ssrRenderComponent(_component_NuxtLink, {
          to: "/catalogue",
          ...
          default: withCtx((_, _push2, _parent2, _scopeId) => {
            if (_push2) {
              ...
              _push2(` Commander à nouveau `);
            } else {
              return [
                createVNode(unref(RefreshCw), { class: "w-5 h-5" }),
                createTextVNode(" Commander à nouveau ")
              ];
            }
          }),
          _: 1
...
//# sourceMappingURL=_id_-DHF0vBc6.js.map
