import { k as useRoute, g as useToast, a as useHead, b as __nuxt_component_3, l as __nuxt_component_0, _ as __nuxt_component_3$1, s as useSupabase } from "../server.mjs";
  ...
  setup(__props) {
    ...
    const fetchOrder = async () => {
      ...
      try {
        ...
        order.value = allOrders?.find((o) => o.id === route.params.id) || null;
        if (order.value) {
          ...
          if (order.value.assigned_to) {
            ...
            assignedLivreur.value = livreurData?.find((l) => l.id === order.value.assigned_to) || null;
          }
        }
      } catch (error) {
    ...
    const getStatusLabel = (status) => {
      const labels = { pending: "En attente", processing: "En cours", completed: "Terminé", cancelled: "Annulé" };
    ...
    const getFulfillmentLabel = (status) => {
      const labels = { not_fulfilled: "Non traité", fulfilled: "Prêt", shipped: "En livraison", delivered: "Livré" };
      return labels[status] || status;
    };
    useHead({
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (unref(loading)) {
        ...
        _push(`</div></div><div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6"><h2 class="font-semibold text-gray-900 mb-4">Articles commandés</h2><div class="space-y-4"><!--[-->`);
        ssrRenderList(unref(orderItems), (item) => {
          ...
          _push(`</div><div class="flex-1"><p class="font-medium text-gray-900">${ssrInterpolate(item.title)}</p><p class="text-sm text-gray-500">Qté: ${ssrInterpolate(item.quantity)}</p></div><div class="text-right"><p class="font-semibold text-gray-900">${ssrInterpolate(formatPrice(item.total))}</p><p class="text-xs text-gray-500">${ssrInterpolate(formatPrice(item.unit_price))} / unité</p></div></div>`);
        });
        ...
        _push(`</div></div><div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6"><div class="flex items-center justify-between mb-4"><h2 class="font-semibold text-gray-900">Livreur assigné</h2>`);
        if (unref(order).assigned_to) {
          _push(ssrRenderComponent(_component_UButton, {
            onClick: ($event) => showAssignModal.value = true,
            color: "gray",
        ...
        if (unref(assignedLivreur)) {
          ...
          _push(`</div><div><p class="font-medium text-gray-900">${ssrInterpolate(unref(assignedLivreur).first_name)} ${ssrInterpolate(unref(assignedLivreur).last_name)}</p><p class="text-sm text-gray-500">${ssrInterpolate(unref(assignedLivreur).phone || "Pas de téléphone")}</p></div></div>`);
        } else {
          ...
          _push(`<p class="text-sm text-gray-500">Aucun livreur assigné</p>`);
          _push(ssrRenderComponent(_component_UButton, {
            onClick: ($event) => showAssignModal.value = true,
            color: "primary",
            size: "sm",
        ...
        if (unref(order).delivery_photo) {
          _push(`<div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6"><h2 class="font-semibold text-gray-900 mb-4">Photo de livraison</h2><img${ssrRenderAttr("src", unref(order).delivery_photo)} alt="Preuve de livraison" class="w-full rounded-lg"><p class="text-xs text-gray-500 mt-2 text-center"> Livrée le ${ssrInterpolate(formatDateTime(unref(order).delivered_at))}</p></div>`);
        } else {
          _push(`<!---->`);
        }
        _push(`</div></div>`);
...
//# sourceMappingURL=_id_-Bh5ffSTK.js.map
