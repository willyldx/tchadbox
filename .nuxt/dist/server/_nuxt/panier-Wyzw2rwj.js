import { f as useCartStore, g as useToast, a as useHead, d as __nuxt_component_0 } from "../server.mjs";
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (unref(cartStore).isEmpty) {
        ...
        _push(`</div><h2 class="text-2xl font-bold text-[var(--color-text)] mb-3">Votre panier est vide</h2><p class="text-[var(--color-text-muted)] mb-8 max-w-sm mx-auto"> Parcourez notre catalogue et faites plaisir à votre famille au Tchad </p>`);
        _push(ssrRenderComponent(_component_NuxtLink, {
          to: "/catalogue",
          class: "btn-gold"
        }, {
          default: withCtx((_, _push2, _parent2, _scopeId) => {
            if (_push2) {
              ...
              _push2(`Découvrir le catalogue</span>`);
            } else {
              return [
                createVNode("span", null, [
                  createVNode(unref(ArrowRight), { class: "w-5 h-5" }),
                  createTextVNode("Découvrir le catalogue")
                ])
              ];
            }
          }),
        ...
        ssrRenderList(unref(cartStore).items, (item) => {
          ...
          if (item.quantity > 1) {
            _push(`<p class="text-xs text-[var(--color-text-muted)]">${ssrInterpolate(formatPrice(item.price))} /pièce</p>`);
          } else {
            _push(`<!---->`);
          }
        ...
        _push(`</div><div class="lg:col-span-1"><div class="card p-6 sticky top-28 space-y-6"><h2 class="text-xl font-bold text-[var(--color-text)]">Résumé</h2><div class="space-y-3"><!--[-->`);
        ssrRenderList(unref(cartStore).items, (item) => {
          _push(`<div class="flex justify-between text-sm"><span class="text-[var(--color-text-secondary)] truncate mr-3">${ssrInterpolate(item.title)} × ${ssrInterpolate(item.quantity)}</span><span class="text-[var(--color-text)] font-medium flex-shrink-0">${ssrInterpolate(formatPrice(item.price * item.quantity))}</span></div>`);
        });
...
//# sourceMappingURL=panier-Wyzw2rwj.js.map
