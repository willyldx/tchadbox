import { j as useRoute, s as useSupabase, g as useToast, a as useHead, b as __nuxt_component_3$1, l as __nuxt_component_0$2, _ as __nuxt_component_3$2, i as useAuthStore } from './server.mjs';
  ...
  setup(__props) {
    ...
    const startDelivery = async () => {
      ...
      try {
        ...
        toast.add({ title: "Succ\xE8s", description: "Livraison d\xE9marr\xE9e", color: "green" });
      } catch (error) {
        console.error("Error starting delivery:", error);
        toast.add({ title: "Erreur", description: "Impossible de d\xE9marrer la livraison", color: "red" });
      } finally {
    ...
    const openInMaps = () => {
      var _a;
      if (!((_a = delivery.value) == null ? void 0 : _a.shippingAddress)) return;
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (unref(loading)) {
        ...
        _push(`</div><div class="flex-1"><p class="text-sm text-gray-500">T\xE9l\xE9phone</p><p class="font-medium text-gray-900">${ssrInterpolate(unref(delivery).recipientPhone)}</p></div>`);
        _push(ssrRenderComponent(_component_Icon, {
        ...
        if (unref(delivery).shippingAddress) {
          ...
          _push(`<p class="text-gray-500">Adresse non sp\xE9cifi\xE9e</p>`);
        }
        if (unref(delivery).shippingAddress) {
          _push(ssrRenderComponent(_component_UButton, {
            onClick: openInMaps,
        ...
        _push(` Articles (${ssrInterpolate(((_a = unref(delivery).items) == null ? void 0 : _a.length) || 0)}) </h2><div class="space-y-3"><!--[-->`);
        ssrRenderList(unref(delivery).items, (item) => {
          ...
          _push(`</div><div class="flex-1"><p class="font-medium text-gray-900 text-sm">${ssrInterpolate(item.title)}</p><p class="text-xs text-gray-500">Qt\xE9: ${ssrInterpolate(item.quantity)}</p></div></div>`);
        });
        ...
        if (unref(delivery).fulfillmentStatus === "delivered") {
          ...
          _push(`<p class="font-medium text-green-700">Livraison effectu\xE9e</p><p class="text-sm text-green-600">${ssrInterpolate(formatDateTime(unref(delivery).deliveredAt))}</p>`);
...
//# sourceMappingURL=_id_-AD7zLEQk.mjs.map
