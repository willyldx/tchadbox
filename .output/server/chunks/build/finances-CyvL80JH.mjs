...
import { i as useAuthStore, n as navigateTo, a as useHead, _ as __nuxt_component_3$2, e as __nuxt_component_0$4 } from './server.mjs';
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    const periodOptions = [
      ...
      { label: "Cette ann\xE9e", value: "year" },
    ...
    const getPaymentLabel = (status) => {
      const map = { captured: "Pay\xE9", awaiting: "En attente", refunded: "Rembours\xE9" };
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (unref(topProducts).length === 0) {
        _push(`<div class="text-center py-4 text-gray-500 text-sm"> Pas encore de donn\xE9es </div>`);
      } else {
        _push(`<!---->`);
      }
      _push(`</div></div></div><div class="bg-white rounded-xl shadow-sm border border-gray-100"><div class="p-6 border-b border-gray-100 flex items-center justify-between"><h2 class="font-semibold text-gray-900">Transactions r\xE9centes</h2>`);
      ...
      if (unref(recentOrders).length === 0) {
        _push(`<div class="p-8 text-center text-gray-500"> Aucune transaction r\xE9cente </div>`);
      } else {
        _push(`<!---->`);
      }
      _push(`</div></div></div>`);
...
//# sourceMappingURL=finances-CyvL80JH.mjs.map
