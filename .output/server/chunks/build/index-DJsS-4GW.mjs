import { d as useSeoMeta, u as useRouter, f as useCartStore, i as useAuthStore, e as __nuxt_component_0$4, k as useRuntimeConfig } from './server.mjs';
import { defineComponent, ref, reactive, computed, withCtx, unref, createVNode, createTextVNode, resolveDynamicComponent, useSSRContext } from 'vue';
...
const usePaystack = () => {
  ...
  const initializePayment = async (options) => {
    ...
    const handler = PaystackPop.setup({
      ...
      // XOF = Franc CFA (BCEAO) — adapté pour le Tchad/diaspora
      ref: options.reference || generateReference(),
      metadata: {
        custom_fields: [
          ...((_a = options.metadata) == null ? void 0 : _a.custom_fields) || []
        ],
        ...options.metadata
      },
  ...
  const verifyPayment = async (reference) => {
    try {
      ...
      return {
        success: false,
        error: error.message || "Erreur de v\xE9rification du paiement"
      };
    }
  };
  const eurToXof = (eurAmount) => {
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    const form = reactive({
      ...
      email: ((_c = authStore.user) == null ? void 0 : _c.email) || "",
      phone: ((_d = authStore.user) == null ? void 0 : _d.phone) || "",
      recipientName: "",
      recipientPhone: "",
      address: {
        address1: "",
    ...
    const paymentMethods = [
      {
        ...
        description: "Visa, Mastercard \u2014 s\xE9curis\xE9 via Paystack",
        icon: CreditCard
      },
      {
        id: "mobile_money",
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      ssrRenderList(steps, (step, index) => {
        ...
        _push(`</div><span class="${ssrRenderClass([index <= unref(currentStep) ? "text-amber-600" : "text-[var(--color-text-muted)]", "text-xs mt-2 font-medium"])}">${ssrInterpolate(step.label)}</span></div>`);
        if (index < steps.length - 1) {
          _push(`<div class="${ssrRenderClass([index < unref(currentStep) ? "bg-amber-500" : "bg-gray-200", "w-20 sm:w-32 h-0.5 mx-2"])}"></div>`);
        } else {
          _push(`<!---->`);
        }
        _push(`</div>`);
      });
      _push(`<!--]--></div></div><div class="grid lg:grid-cols-3 gap-8"><div class="lg:col-span-2"><div class="space-y-6" style="${ssrRenderStyle(unref(currentStep) === 0 ? null : { display: "none" })}"><div class="bg-white rounded-2xl border border-[var(--color-border)] overflow-hidden"><div class="p-6 border-b border-[var(--color-border)]"><h2 class="text-lg font-semibold text-[var(--color-text)]">Vos informations</h2></div><div class="p-6 space-y-5">`);
      if (!unref(authStore).isAuthenticated) {
        ...
        _push(`<div><p class="text-sm text-amber-800 font-medium">D\xE9j\xE0 client ?</p><p class="text-sm text-amber-600">`);
        _push(ssrRenderComponent(_component_NuxtLink, {
      ...
      _push(`<div><p class="text-sm text-green-800 font-medium">Paiement 100% s\xE9curis\xE9</p><p class="text-sm text-green-600 mt-1"> Vos informations de paiement sont trait\xE9es de mani\xE8re s\xE9curis\xE9e par Paystack. Nous ne stockons jamais vos donn\xE9es bancaires. </p></div></div></div></div>`);
      ...
      _push(ssrRenderComponent(_component_NuxtLink, {
        ...
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            _push2(`conditions g\xE9n\xE9rales de vente`);
          } else {
            return [
              createTextVNode("conditions g\xE9n\xE9rales de vente")
            ];
          }
        }),
        _: 1
      ...
      _push(` SSL s\xE9curis\xE9 </span><span class="flex items-center gap-1">`);
      _push(ssrRenderComponent(unref(ShieldCheck), { class: "w-4 h-4" }, null, _parent));
      _push(` Paiement prot\xE9g\xE9 </span></div></div></div></div></div></div>`);
    };
  }
});
...
//# sourceMappingURL=index-DJsS-4GW.mjs.map
